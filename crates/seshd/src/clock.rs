...
//!
//! **Nothing in SESH waits for the clock**, and the reason is measured rather
//! than a matter of taste. The plan originally had startup pause up to ten
//! seconds before reconciling, on the argument that reconciliation has no
//! caller and so no deadline. It has one: `~/.config/labwc/autostart` polls for
//! `seshd` for 10.48s before pointing Chromium at it regardless, and `seshd`
//! otherwise binds 0.69s after starting. A ten-second pause puts the bind at
//! 10.69s and lands the kiosk on a connection-refused page — the black screen
//! PR #4 existed to remove. A ceiling small enough to be safe would not help
//! either: the two boots measured had NTP land 9.2s and 20.8s after `seshd`
//! started, so a safe wait would time out and mark the row anyway.
//!
//! So the mark does the whole job, which is what the spec asked for.
//!
